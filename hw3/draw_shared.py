# Full implementation for visualizing the data
import pandas as pd
import matplotlib.pyplot as plt
from io import StringIO

# Define the log data as a string (shortened for brevity)
log_data = """
...
data = pd.read_csv(StringIO(log_data))

# Plot 1: Interactions_Per_Second_Billion vs nBodies for different BLOCK_SIZE
plt.figure(figsize=(12, 6))
for block_size in data['BLOCK_SIZE'].unique():
    subset = data[data['BLOCK_SIZE'] == block_size]
    plt.plot(subset['nBodies'], subset['Interactions_Per_Second_Billion'], marker='o', label=f'BLOCK_SIZE {block_size}')

plt.title('Interactions_Per_Second_Billion vs nBodies (by BLOCK_SIZE)')
plt.xlabel('nBodies')
plt.ylabel('Interactions_Per_Second_Billion')

plt.yscale('log')  # Log scale for y-axis
plt.legend()
plt.grid(True)
plt.show()
plt.savefig('outputs/shared_analysis_1.png')
plt.savefig('outputs/shared_analysis_1.pdf')

# Plot 2: Interactions_Per_Second_Billion vs nBodies for different BLOCK_STRIDE
plt.figure(figsize=(12, 6))
for block_stride in data['BLOCK_STRIDE'].unique():
    subset = data[data['BLOCK_STRIDE'] == block_stride]
    plt.plot(subset['nBodies'], subset['Interactions_Per_Second_Billion'], marker='o', label=f'BLOCK_STRIDE {block_stride}')

plt.title('Interactions_Per_Second_Billion vs nBodies (by BLOCK_STRIDE)')
plt.xlabel('nBodies')
plt.ylabel('Interactions_Per_Second_Billion')
...
plt.grid(True)
plt.show()
plt.savefig('outputs/shared_analysis_2.png')
plt.savefig('outputs/shared_analysis_2.pdf')


filtered_data = data[data['nBodies'] == 4096]
# Plot: Interactions_Per_Second_Billion vs BLOCK_SIZE for fixed nBodies = 4096, with different BLOCK_STRIDE
plt.figure(figsize=(12, 6))
for block_stride in filtered_data['BLOCK_SIZE'].unique():
    subset = filtered_data[filtered_data['BLOCK_SIZE'] == block_stride]
    plt.plot(subset['BLOCK_STRIDE'], subset['Interactions_Per_Second_Billion'], marker='o', label=f'BLOCK_SIZE {block_stride}')

plt.title('Interactions_Per_Second_Billion vs BLOCK_STRIDE (Fixed nBodies=4096, by BLOCK_SIZE)')
plt.xlabel('BLOCK_STRIDE')
plt.ylabel('Interactions_Per_Second_Billion')
plt.legend()
plt.grid(True)
plt.savefig('outputs/shared_analysis_3.png')
plt.savefig('outputs/shared_analysis_3.pdf')
plt.show()