...
def tokenize(doc):
    ...
    assert (0 <= tokens_np).all() and (tokens_np < 2**16).all(), "token dictionary too big"
...
with mp.Pool(nprocs) as pool:
    ...
    for tokens in pool.imap(tokenize, fw, chunksize=16):
        ...
        if token_count + len(tokens) < shard_size:
            ...
            # split the document into whatever fits in this shard; the remainder goes to the next shard
